...
export const destinationResolvers: IResolvers = {
    ...
    Destination: {
        ...
        categories: async (destination: Destination, _, { pgPool, loaders }: Context) => {
            const result = await pgPool.query(
                'SELECT category_id FROM destination_categories WHERE destination_id = $1',
                [destination.id]
            );
            return loaders.categoryLoader.loadMany(result.rows.map(row => row.category_id));
        },
